...
const canvas = $('c');
const ui = { count:$('count'),cv:$('cv'),speed:$('speed'),sv:$('sv'),attr:$('attr'),av:$('av'),reset:$('reset'),mc:$('mc'),mcv:$('mcv'),ms:$('ms'),msv:$('msv'),ma:$('ma'),mav:$('mav'),mreset:$('mreset'),toggle:$('toggle'),mp:$('mp'),fs:$('fs'),fps:$('fps'),sim:$('sim'),tip:$('tip') };
...
function updateDesktopCount(v){simulation.setParams({count:v});ui.cv.textContent=v;ui.fs.textContent=v}
function updateDesktopSpeed(v){const speed=v/100;simulation.setParams({speed});ui.sv.textContent=speed.toFixed(1)+'×'}
function updateDesktopAttraction(v){const attraction=v/100;simulation.setParams({attraction});ui.av.textContent=attraction.toFixed(2)}
function syncMobile(){const count=+ui.mc.value,speed=+ui.ms.value/100,attraction=+ui.ma.value/100;simulation.setParams({count,speed,attraction});ui.mcv.textContent=count;ui.msv.textContent=speed.toFixed(1)+'×';ui.mav.textContent=attraction.toFixed(2);ui.fs.textContent=count}
function reset(){simulation.reset();ui.fs.textContent=simulation.count}
...
addEventListener('mousemove',pointerMove,{passive:true});addEventListener('mousedown',e=>{if(e.button===0)setPointerDown(e.clientX,e.clientY,1)});addEventListener('mouseup',setPointerUp);addEventListener('mouseleave',()=>{simulation.mouse.inside=false;simulation.mouse.down=false;simulation.mouse.speed=0;pointerDot.classList.remove('visible')});addEventListener('click',e=>{if(e.target===canvas)simulation.addRipple(e.clientX,e.clientY,.7)});addEventListener('touchstart',e=>{lastPointer=null;pointerMove(e);setPointerDown(e.touches[0].clientX,e.touches[0].clientY,1)},{passive:true});addEventListener('touchmove',pointerMove,{passive:true});addEventListener('touchend',()=>{setPointerUp();simulation.mouse.inside=false;lastPointer=null},{passive:true});addEventListener('blur',()=>{setPointerUp();pointerDot.classList.remove('visible')});addEventListener('resize',resize,{passive:true});
