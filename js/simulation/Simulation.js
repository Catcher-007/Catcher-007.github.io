...
export class Simulation {
  ...
  step(dt = 1) {
    ...
    Boids.update(this.fish, this.grid, 60);

    for (const f of this.fish) {
      this.#interact(f);
      f.update(this.speed, dt);
      f.edge();
    }
  }
  ...
  #interact(f) {
    ...
    if (this.mouse.down && d2 > 64) {
      ...
      const force = this.attraction * Math.min(1, 140 * inv);
      ax += dx * inv * force;
      ay += dy * inv * force;
      f.limit = f.max * 1.45;
    } else {
      if (this.mouse.inside && d2 < 14400 && d2 > 0) {
        const d = Math.sqrt(d2);
        const force = (1 - d / 120) * .34;
        ax -= dx / d * force;
        ay -= dy / d * force;
      }
      f.limit = f.max;
    ...
    for (const r of this.ripples) {
      ...
      if (rd2 < rr * rr) {
        ...
        const ring = Math.abs(d - r.r);
        if (ring < 34) {
          const force = (1 - ring / 34) * r.power * .012;
          ax += rx / d * force;
          ay += ry / d * force;
        }
      }
